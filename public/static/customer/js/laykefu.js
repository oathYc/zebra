...
;!function(win){
    ...
    var Laykefu = function(){
        ...
        return JSON.parse(localStorage.getItem(key));//localStorage:设置本地存储
        ...
        socket.onmessage = function (res) {
            ...
            switch(data['message_type']){
                ...
                case 'wait':
                    ...
                    $('#laykefu').addClass('filt');
                    var isMsg = "<div class='laykefu-isMsg' id='laykefu-isMsg' style='width:250px;'>";
                    isMsg +=    "<div class='laykefu-isMsg-heading'>";
                    isMsg +=        "<span class='laykefu-isMsg-title'>";
                    isMsg +=            '提示';
                    isMsg +=        '</span>';
                    isMsg +=        "<span class='laykefu-isMsg-close laykefu-right'>";
                    isMsg +=        '</span>';
                    isMsg +=    '</div>';
                    isMsg +=    "<div class='laykefu-isMsg-content'>";
                    isMsg +=        data.data.content;
                    isMsg +=    '</div>';
                    isMsg +=    '<div class="input-group laykefu-right">';
                    isMsg +=        '<span class="input-group-btn">';
                    isMsg +=            '<button class="btn btn-primary sure-msg" type="button">我要留言</button>';
                    isMsg +=        '</span>';
                    isMsg +=    '</div>';
                    isMsg += "</div>";
                    $(isMsg).prependTo('body');

                    var msgBox = "<div class='laykefu-msgBox' style='display:none;max-width:640px;min-width:360px;'>";
                    msgBox +=   "<form action='#' method='POST' id='form1' onsubmit='return false;'>";
                    msgBox +=       "<div class='laykefu-msgBox-heading'>";
                    msgBox +=           "<span class='laykefu-msgBox-title'>";
                    msgBox +=                 '请您留言';
                    msgBox +=           '</span>';
                    msgBox +=           "<span class='laykefu-msgBox-close laykefu-right'></span>";
                    msgBox +=       '</div>';
                    msgBox +=       "<div class='laykefu-msgBox-content' style='height:440px;'>";
                    msgBox +=           "<div class='msgBox-tip'>";
                    msgBox +=               '<span>您好，很抱歉我们暂时无法为您提供服务，如需帮助，请留言，我们将尽快联系并解决您的问题</span>';
                    msgBox +=           '</div>';
                    msgBox +=           "<div class='msgBox-wrap'>";
                    msgBox +=               "<div class='msgBox-part-title'>";
                    msgBox +=                   '<span>*</span>';
                    msgBox +=                   '<span>问题描述</span>';
                    msgBox +=               '</div>';
                    msgBox +=               "<div class='msgBox-part-content'>";
                    msgBox +=                   '<textarea placeholder="问题描述(200字以内)" maxlength="200" name="msg"></textarea>';
                    msgBox +=               '</div>';
                    msgBox +=           '</div>';
                    msgBox +=           "<div class='msgBox-wrap'>";
                    msgBox +=               "<div class='msgBox-part-title'>";
                    msgBox +=                   "<span></span>";
                    msgBox +=                   "<span>上传图片</span>";
                    msgBox +=               '</div>';
                    msgBox +=               "<div class='msgBox-part-content'>";
                    msgBox +=                   "<ul class='upload-ul clearfix'>";
                    msgBox +=                       "<li class='upload-pick'>";
                    msgBox +=                           "<div class='webuploader-container clearfix' id='goodsUpload'>";
                    msgBox +=                            '</div>';
                    msgBox +=                        '</li>';
                    msgBox +=                     '</ul>';
                    msgBox +=                '</div>';
                    ...
                    msgBox +=               '</div>';
                    msgBox +=               "<div class='msgBox-part-content'>";
                    msgBox +=                   "<input type='text' name='mailbox' />";
                    msgBox +=               '</div>';
                    msgBox +=            '</div>';
                    msgBox +=       '</div>';
                    msgBox +=       "<div class='laykefu-msgBox-footer'>";
                    msgBox +=                   "<button class='btn btn-primary' >提交留言</button>";
                    msgBox +=       '</div>';
                    msgBox +=   '</form>';
                    msgBox += '</div>';
                    $(msgBox).prependTo('body');
                    $('body').prepend("<div class='loader'>loading</div>");

                    let tip = "<div class='laykefu-tip'>";
                    tip +=    "</div>";
                    $(tip).prependTo('body');
                ...
                case 'relinkMessage':
                    ...
                    break;
                // 转接
                case 'longNotOperating':
                    showSystem({content: data.data.content});
                    // 滚动条自动定位到最底端
                    wordBottom();
                    break;
            }
    ...
    },accurateTimers = function (fn, expectDelayTime) {
        let init = false;
        ...
        var tpl  = "<div style= 'width:" + config.width + ";height:"+ config.height+";' class='tpl' id='laykefu'>";
        tpl +=      '<div class="small-chat-box fadeInRight animated">';
        tpl +=          '<div class="heading" draggable="true" style="height:16px;display:flex;flex-direction:row;align-items:center;justify-content:space-between;">';
        tpl +=              '<span class="laykefu-title" id="laykefu-title">与 客服 交流中</span>'
        tpl +=              '<small class="chat-date laykefu-close" id="hidden-laykefu" title="关闭">';
        tpl +=              '</small>';
        tpl +=              '<div class="link-group">';
        tpl +=                  '<a href="javascript:;" style="margin-right:10px;color:#555;text-decoration:underline;">热门推荐</a>';
        tpl +=                  '<a href="javascript:;" style="color:#555;text-decoration:underline;">下载链接</a>';
        tpl +=              '</div>';
        tpl +=          '</div>';
        ...
        if( navigator.userAgent.match(/Android/i)
            ...
            $('#laykefu-max').css('display','none');
            $('#hidden-laykefu').css('display','none');
            $(".laykefu-msgBox").css({'width':'100%'});
            $(".laykefu-msgBox-content").css({'height':'calc(100vh - 183px)'});
            $(".float-window").hide();
            $('.link-group').show();
            // $('.laykefu-more').show();
        }else{
            $('#hidden-laykefu').css('display','block');
            $("#laykefu").css({'width':'70%','height':'600px','max-width':'640px','min-width':'360px'});
            $('.tpl').css({'margin-top':'calc(50vh - 300px)'});
            $('.laykefu-msgBox').css({'top':'calc(50vh - 312px)'});
            $(".float-window").show();
            $('.link-group').hide();
            // $('.laykefu-more').hide();
        ...
        // laykefu-nav-item laykefu-nav-child
        $(".laykefu-nav-item a img").unbind('click').click(function(){

            if($('.laykefu-nav-item .laykefu-nav-child').is(':visible')){
                $('.laykefu-nav-item .laykefu-nav-child').fadeOut().css({'display': 'none'});
            } else {
                $('.laykefu-nav-item .laykefu-nav-child').fadeIn().css({'display': 'flex'});
            }
        });

        $(".float-window .float-window-list .item").hover(function(){
            $(this).children('img').hide();
            $(this).children('.show-text').css({'display':'flex'});
        },function(){
        ...
        //点击提示框的关闭按钮
        $('.laykefu-isMsg-close').click(function(){
            $('.laykefu-isMsg').hide();
            $('#laykefu').removeClass('filt');
        });
        //点击我要留言按钮
        ...
        $('.laykefu-msgBox-close').click(function(){
            $('.laykefu-msgBox').hide();
            $('.float-window').show();
            $('#laykefu').removeClass('filt');
        });
        $('.laykefu-msgBox-footer button').click(function(){

            var msgVal = $(".laykefu-msgBox-content textarea").val();
            var mailbox = $(".laykefu-msgBox-content input[name='mailbox']").val();

            if(!msgVal){
                $(".laykefu-msgBox-content textarea").css({'border-color':'red'});
                $(".laykefu-msgBox-content textarea").parent().prev('.msgBox-part-title').css({'color':'red'});
            }else{
                $(".laykefu-msgBox-content textarea").css({'border-color':'#a9a9a9'});
                $(".laykefu-msgBox-content textarea").parent().prev('.msgBox-part-title').css({'color':'#777'});
            }

            if(!mailbox || !emailReg.test(mailbox)){
                $(".laykefu-msgBox-content input[name='mailbox']").css({'border-color':'red'});
                $(".laykefu-msgBox-content input[name='mailbox']").parent().prev('.msgBox-part-title').css({'color':'red'});
            } else {
                $(".laykefu-msgBox-content input[name='mailbox']").css({'border-color':'#a9a9a9'});
                $(".laykefu-msgBox-content input[name='mailbox']").parent().prev('.msgBox-part-title').css({'color':'#777'});
            }

            if(msgVal && mailbox && emailReg.test(mailbox)){
            ...
            function validateToken() {
                ...
                if(localStorage.getItem('accessToken') === null){
                    ...
                    let paramsSeri = MXHY.getParams();//参数序列化

                    MXHY.send(httpVar + token, 'GET', paramsSeri, (data, msg, code)=> {
                        if ( code === 200 ) {
                            saveLocalVar(data.accessToken, data.expiresTime, data.refreshToken);
                            imgSend();
                ...
                } else if(new Date() > new Date(+localStorage.getItem('expiresTime'))) {

                    let refreshToken = {
                        refreshToken:localStorage.getItem('refreshToken')
                    };
                    let paramsSeri = MXHY.getParams(refreshToken);

                    MXHY.send(httpVar + refresh, 'GET', paramsSeri , (data, msg, code)=> {
                        if(code === 200) {
                            saveLocalVar(data.accessToken, data.expiresTime, data.refreshToken);
                            imgSend();
            ...
            /*保存localStorage变量*/
            function saveLocalVar(accessToken, expiresTime, refreshToken) {
                localStorage.setItem('accessToken', accessToken);
                localStorage.setItem('expiresTime',  expiresTime+'000');//+((new Date().getTime()) + (1*60*1000))
            ...
            function imgSend(){
                let img;
                if ($('ul.upload-ul').find('.viewThumb').length > 0) {
                    img = $('.viewThumb').children('img').prop('src');
                } else {
                    img = '';
                }
                if(img){
                    let newObj = {
                        appId:localStorage.getItem('accessToken'),
                        imgBs:img
                    };

                    let paramsSeri = MXHY.getParams(newObj);

                    MXHY.send(httpVar + userImg, 'POST', paramsSeri, (data, msg, code)=>{
                        if (code === 200) {
                            msgSend(data.imgBs);
                        }else{
                            msgTip(msg);
            ...
            /*提交留言*/
            function msgSend(img) {
                let params = {
                    'msg' : $("#form1 textarea[name='msg']").val().replace(/\s+/g,"").replace(/\"/g,"'"),//去掉字符串中的所有空格
                    'mailbox' : $("#form1 input[name='mailbox']") .val().replace(/\s+/g,""),
                    'appId' : localStorage.getItem('accessToken'),
                    'img' : img
                };
                let paramsSeri = MXHY.getParams(params);

                MXHY.send(httpVar + addMessage, 'POST', paramsSeri, (data, msg, code)=>{
                    if(code === 200){
                        let isSuccess = true;
                        $('.viewThumb').children('img').prop('src',img);
                        msgTip(msg, isSuccess);
            ...
            /*消息提示*/
            function msgTip(msg,isSuccess) {
                $('.loader').hide();
                $('.laykefu-tip').html(msg);
                $('.laykefu-tip').css({
                    position:'absolute',
                    top:'0',
                    right:'0',
                ...
                setTimeout("$('.laykefu-tip').animate({opacity:.5,padding:'20px 29px'},70).animate({opacity:0,padding:'1px'},120);",2000);
                setTimeout("$('.laykefu-tip').hide()",2190);

                if(isSuccess){
                    setTimeout('window.location.reload();',2190);
                }
            }
        });
