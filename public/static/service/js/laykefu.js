...
function showUserMessage(uinfo, content) {
    ...
    if (!$('#f-' + uinfo.id).hasClass('active')) {
        ...
        //设置未读条数本地缓存
        localStorage.setItem('uinfoItem'+ uinfo.id, num);
...
function addUser(data) {
    ...
    if (isExist === +0){
        // 判断该记录是否有未读消息
        let num = localStorage.getItem('uinfoItem'+ data.id) === null ? 0 : localStorage.getItem('uinfoItem'+ data.id);
        ...
        var _html = '<li class="layui-nav-item" data-id="' + data.id + '" id="f-' + data.id +
            ...
            _html += '<span class="layui-badge" style="margin-left:5px"> ' + num + '</span>';
    ...
    if(0 == hasActive){
        ...
        //去除未读消息提示的本地缓存
        if(localStorage.getItem('uinfoItem' + uid) !== null){
            localStorage.removeItem('uinfoItem' + uid);
        }
