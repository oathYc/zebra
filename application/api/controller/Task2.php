...
class Task2 extends Controller
{
    ...
    public function passRewardSend()
    {
        ...
        foreach ($pass as $k => $v) {
            ...
            $allJoin = db('three_pass_join')->where(['passId' => $v['id'], 'number' => $number, 'isReward' => 0])->select();
            $userSign = [];//用户签到信息
...
//               $rewardMoney = $v['money'] * ($v['reward']/100);
            ...
            $rewardMoney = Share2::getDecimalMoney($rewardMoney);
            //奖励发放
...
//                   $userMoney = $rewardMoney*intval($challengeNumber);//按挑战轮数计算
                ...
                $isReward = db('three_pass_reward')->where(['uid' => $uid, 'passId' => $v['id'], 'joinId' => $joinId, 'date' => $checkDate])->find();
                if ($isReward) {
//                   修改对应的奖励发送状态
                    db('three_pass_join')->where('id', $joinId)->update(['isReward' => 1]);//奖励状态
                    continue;
                }
                //奖励发放
                Share2::sendPassRewardNew($uid, $userMoney, $v, $joinId, $number);
                try {
                    if ($userNumberData) {//记录用户的每轮奖励数据
                        db('three_pass_reward_number')->insertAll($userNumberData);
                    }
                    //邀请人分销奖励发放
                    Share2::sendPassShareReward($uid, $userMoney, $v, $joinId, $number);
                    if ($challengeNumber == $v['challenge']) {//闯关挑战全部完成 额外奖励发送
                        Share2::sendPassFinishReward($uid, $v, $joinId, $number);
                    }
                } catch (Exception $re) {
                }
                //本金退还
                if ($return == 1) {
                    Share2::returnPassJoinMoney($uid, $v['money'], $v['name']);
                }
                //修改对应的奖励发送状态
                db('three_pass_join')->where('id', $joinId)->update(['isReward' => 1]);//奖励状态
            }
...
//               db('three_pass')->where('id',$v['id'])->update(['status'=>0,'isEnd'=>1]);
//           }
        }
    }
...
//                    $rewardTotalMoney = self::getRewardTotalMoney($failMoney);
                ...
                foreach ($successSign as $w => $q) {
                    ...
                    $uid = $q['uid'];
                    //金额规范  分
                    $money = Share2::getDecimalMoney($rewardMoney);
                    $user = db('member')->where('id', $uid)->find();
                    $addMoney = $user['money'] + $money;
                    db('member')->where('id', $uid)->update(['money' => $addMoney]);
                    Share2::userMoneyRecord($uid, $money, '打卡活动每日奖励' . '-' . $v['name'], 1, 1, 1);
                    Share2::rewardRecord($uid, $money, $v['id'], 1, $q['joinId']);
                    Share2::userMoneyGet($uid, $money, 1);
                    $successSignUid[] = $uid;
                }
                //记录奖励记录
                $param = [
    ...
    public static function getRewardTotalMoney($money)
    {
        ...
        return Share2::getDecimalMoney($returnMoney);
    }
    ...
    public static function getUserSignNumber($uid, $passId, $joinId)
    {
        ...
        $hadSign = db('three_pass_sign')->where(['uid' => $uid, 'passId' => $passId, 'joinId' => $joinId, 'status' => 1, 'is_true' => 1])->order('number')->count();
        if ($hadSign) {
            return $hadSign;
        } else {
            return 0;
    ...
    public static function getUserChallengeSuccess($join)
    {
        ...
        if ($join['status'] == 1) {
            $signError = db('three_pass_sign')->where(['uid' => $join['uid'], 'passId' => $join['passId'], 'joinId' => $join['id'], 'status' => 2])->find();
            //是否有签到失败的数据
            if ($signError) {
                db('three_pass_join')->where('id', $join['id'])->update(['status' => 2]);//参加状态  0-参与中 1-已完成 2-未完成
                return 0;
            }
            return 1;
        }
        if ($join['status'] == 0) {
            ...
            if ($join['signStatus'] == 0 || $join['signStatus'] == 1) {
                ...
                //判断是否有未签到的记录
                $noSign = db('three_pass_sign')->where(['uid' => $join['uid'], 'passId' => $join['passId'], 'joinId' => $join['id'], 'status' => 0])->find();
                if ($noSign) {//存在 判断当前签到的结束时间 已过签到结束时间说明挑战失败 未过咋忽略该轮挑战签到  闯关暂停引起的
                    ...
                    db('three_pass_join')->where('id', $join['id'])->update(['status' => $status]);
                    return $success;
                } else {//没有
                    //判断是否有签到失败的数据
                    $signError = db('three_pass_sign')->where(['uid' => $join['uid'], 'passId' => $join['passId'], 'joinId' => $join['id'], 'status' => 2])->find();
//                    没有说明都签到成功
    ...
    public static function checkPassJoin()
    {
        ...
        $joinData = db('three_pass_join')->where(['status' => 0])->select();
        foreach ($joinData as $k => $v) {
            //签到状态 signStatus 参与签到状态 0-暂停 1-停止（挑战结束） 2-下一轮（继续挑战）
            if ($v['signStatus'] == 2) {//判断最近一轮签到是否未签到且过签到结束时间
                $noSign = db('three_pass_sign')->where(['uid' => $v['uid'], 'passId' => $v['passId'], 'joinId' => $v['id'], 'status' => 0])->find();
                $now = date('Y-m-d H:i:s');
                if ($noSign && $now > $noSign['signTimeEnd']) { //有待签到且已过签到结束时间
                    db('three_pass_join')->where('id', $v['id'])->update(['status' => 2]);
    ...
    public static function sendRoomReward($room)
    {
        ...
        foreach ($successData as $k => $v) {
            ...
            //余额记录添加
            Share2::userMoneyRecord($v['uid'], $userMoney, '房间挑战奖励发放-' . $room['name'], 1, 2, 1);
            //收益记录
            Share2::userMoneyGet($v['uid'], $userMoney, 2);
            Share2::rewardRecord($v['uid'], $userMoney, $room['id'], 2, $v['id']);
            //本金退还
            $returnMoney = $addMoney + $v['joinMoney'];
            db('member')->where('id', $v['uid'])->update(['money' => $returnMoney]);
            Share2::userMoneyRecord($v['uid'], $v['joinMoney'], '房间挑战报名费退还-' . $room['name'], 1, 2);
            //记录用户的占比数据
    ...
    public static function getUserPassReward($uid, $passId, $joinId, $joinMoney = 0)
    {
        ...
        $passPercent = db('three_pass_percent')->where('passId', $passId)->order('number', 'asc')->select();
        if (!$joinMoney || !$passPercent) {
            return $return;
        }
        //获取签到成功的数据 打卡状态  0-未打卡 1-已打卡
        $signSuccessData = db('three_pass_sign')->where(['uid' => $uid, 'passId' => $passId, 'joinId' => $joinId, 'status' => 1])->order('number', 'asc')->select();
        if (!$signSuccessData) {
            return $return;
        }
        $date = date('Y-m-d');
        ...
        foreach ($signSuccessData as $k => $v) {
            ...
            foreach ($passPercent as $p => $o) {
                if ($o['number'] == $number) {
                    ...
                    $money = Share2::getDecimalMoney($money);
                    //记录数据
